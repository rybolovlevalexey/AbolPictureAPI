...
from app.crud import CrudUserActions
...
async def post_register_new_user(user: User):
    check_user_res = await CrudUserActions.check_user_name_in_db(user.username)
    if check_user_res:
    ...
    await CrudUserActions.add_new_user(**db_user.model_dump())
...
async def post_login_user(user: User):
    db_user = await CrudUserActions.get_user_by_name(user.username)
